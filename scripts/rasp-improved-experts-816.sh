...
for DATASET in "${TASKS[@]}"; do
  ...
  case "$DATASET" in
      hist)         VOCAB=8  MAX=16  L=1 H=4 M=2 ;;
      double_hist)  VOCAB=8  MAX=16  L=3 H=4 M=2 ;;   # 2 cat + 2 num heads
      reverse)      VOCAB=8  MAX=16  L=3 H=8 M=2 ;;
      sort)         VOCAB=8  MAX=16  L=3 H=8 M=4 ;;
      most_freq)    VOCAB=8  MAX=16  L=4 H=8 M=4 ;;
  esac
  # ──────────────────────────────────────────────────────────────────────────
  ...
  python src/run.py \
      ...
      --save --save_code \
      --output_dir "output-improved/rasp-improved-experts-816/${DATASET}/k${VOCAB}_len${MAX}_L${L}_H${H}_M${M}/s${SEED}"
done