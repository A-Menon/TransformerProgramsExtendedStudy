...
# usage: bash rasp-improve.sh [comma_separated_improvements]
set -euo pipefail

IMPROV=${1:-""}
if [ -n "$IMPROV" ]; then
  IMP_ARG="--improvements $IMPROV"
  OUT_SUBDIR="$IMPROV"
else
  IMP_ARG=""
  OUT_SUBDIR="base"
...
SEED=0; DATASET_SIZE=20000; BATCH=512; EPOCHS=250
LR=2e-2; CLIP=1.0; GUMBEL_SAMPLES=4
TAU_INIT=3.0; TAU_END=0.1; TAU_SCHED=geomspace
...
for DATASET in "${TASKS[@]}"; do
  ...
  ATT_TYPE="both"; COUNT_ONLY="--count_only"; MLP_TYPE="--mlp_type cat"

  case "$DATASET" in
    hist)
      ATT_TYPE="num"; N_HEADS_CAT=0; N_HEADS_NUM=$H ;;
    double_hist)
      ATT_TYPE="both"; COUNT_ONLY=""
      N_HEADS_CAT=2; N_HEADS_NUM=2
      N_CAT_MLPS=1;  N_NUM_MLPS=1
      MLP_TYPE="--mlp_type mix" ;;
  esac

  echo -e "\n=== $DATASET (improvements=$IMPROV) ==="
  python src/run.py \
    --dataset "$DATASET" \
    --vocab_size "$VOCAB" \
    ...
    --save --save_code \
    $IMP_ARG \
    --output_dir "output-improved/rasp-improved/${OUT_SUBDIR}/${DATASET}/k${VOCAB}_len${MAX}_L${L}_H${H}_M${M}/s${SEED}"
done